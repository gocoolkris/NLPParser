...
public class ContentTokenizer {
	...
	SegmentedRawContent doc;
	TokenizedRawContent trc;
	int position = 0;
	Lemmatizer lemmatizer;
	public ContentTokenizer(SegmentedRawContent src){
		doc = src;
		trc = new TokenizedRawContent(src.getUrl());
		lemmatizer = new Lemmatizer();
	}

	public TokenizedRawContent getTokenizedRawContent(){
		try{
			return trc;
		}catch(Exception e){}
		return null;
	...
	public void tokenizeSegmentedRawContent(){
		try{
			tokenizeTitle();
			tokenizeHeaderMetaData();
			tokenizeSectionHeaders();
			tokenizeHighlightedText();
			tokenizeRemainingText();
		}catch(Exception e){}
	}

	public void tokenizeHeaderMetaData(){
		try{
			String[] headerMetadataTokens = tokenizeElements(doc.getHeaderMetadata());
			ArrayList<Token> headermetadataTokenList = new ArrayList<Token>();
			for(String s : headerMetadataTokens){
				++position;
				List<String> splitwords = lemmatizer.lemmatize(s);
				for(String word : splitwords){
					if(!documentStopWords.contains(word.toLowerCase().trim()) && 
							!punctuationFilterWords.contains(word.toLowerCase().trim())){
						Token t = new Token(word.toLowerCase().trim(), position);
						headermetadataTokenList.add(t);
					}
				}
			}
			trc.setHeaderMetaDataTokens(headermetadataTokenList);
		}catch(Exception e){}
	}
	...
	public void tokenizeTitle(){
		try{
			String[] titleTokens = tokenizeElements(doc.getTitle());
			ArrayList<Token> titleTokensList = new ArrayList<Token>();
			for(String s : titleTokens){
				++position;
				List<String> splitwords = lemmatizer.lemmatize(s);
				for(String word : splitwords){
					if(!documentStopWords.contains(word.toLowerCase().trim()) && 
							!punctuationFilterWords.contains(word.toLowerCase().trim())){
						Token t = new Token(word.toLowerCase().trim(), position);
						titleTokensList.add(t);
					}
				}
			}
			trc.setTitleTokens(titleTokensList);
		}catch(Exception e){}
	}

	public void tokenizeSectionHeaders(){
		try{
			String[] sectionHeaderTokens = tokenizeElements(doc.getSectionHeaders().toString());
			ArrayList<Token> sectionHeaderTokensList = new ArrayList<Token>();
			for(String s : sectionHeaderTokens){
				++position;
				List<String> wordsplit = lemmatizer.lemmatize(s);
				for(String word: wordsplit){
					if(!documentStopWords.contains(word.toLowerCase().trim()) && 
							!punctuationFilterWords.contains(word.toLowerCase().trim())){
						Token t = new Token(word.toLowerCase().trim(), position);
						sectionHeaderTokensList.add(t);
					}
				}
			}
			trc.setSectionHeaderTokens(sectionHeaderTokensList);
		}catch(Exception e){}
	}

	public void tokenizeHighlightedText(){
		try{
			String[] highlightTokens = tokenizeElements(doc.getHighlightText().toString());
			ArrayList<Token> highlightTokensList = new ArrayList<Token>();
			for(String s : highlightTokens){
				++position;
				List<String> wordsplit = lemmatizer.lemmatize(s);
				for(String word: wordsplit){
					if(!documentStopWords.contains(word.toLowerCase().trim()) && 
							!punctuationFilterWords.contains(word.toLowerCase().trim())){
						Token t = new Token(word.toLowerCase().trim(), position);
						highlightTokensList.add(t);
					}
				}
			}
			trc.setHighlightedTextTokens(highlightTokensList);
		}catch(Exception e){}
	}

	public void tokenizeRemainingText(){
		try{
			String[] remainingText = tokenizeElements(doc.getHighlightText().toString());
			ArrayList<Token> remainingTextTokensList = new ArrayList<Token>();
			for(String s : remainingText){
				++position;
				List<String> wordsplit = lemmatizer.lemmatize(s);
				for(String word : wordsplit){
					if(!documentStopWords.contains(word.toLowerCase().trim()) && 
							!punctuationFilterWords.contains(word.toLowerCase().trim())){
						Token t = new Token(word.toLowerCase().trim(), position);
						remainingTextTokensList.add(t);
					}
				}
			}
			trc.setRemainingTextTokens(remainingTextTokensList);
		}catch(Exception e){}
	}

	private String[] tokenizeElements(String contents){
		try{
			System.out.println("contents received :" + contents);
			InputStream is = ContentTokenizer.class.getResourceAsStream("/en-token.bin");
	...
	public void print(){
		print(trc.getTitleTokens());
		print(trc.getHeaderMetaDataTokens());
		print(trc.getSectionHeaderTokens());
		print(trc.getHighlightedTextTokens());
	}

	private void print(ArrayList<Token> tokenList){
		if(tokenList != null){
	...
	private static HashSet<String> documentStopWords = new HashSet<String>(Arrays.asList(new String[]{
			"a","a's","able","about","above","according","accordingly",
			"across","actually","after","afterwards","again","against",
			"ain't","al","all","allow","allows","almost","alone","along",
			...
			"e","each","edu","eg","eight","either","else",
			"elsewhere","enough","entirely","especially","et",
			...
			"g","get","gets","getting","given","gives","go",
			"goes","going","gone","got","gotten","greetings","h",
			...
			"taken","tell","tends","th","than","thank","thanks","thanx",
			...
			"wouldn't","x","y","years","yes","yet","you","you'd","you'll",
			"you're","you've","your","yours","yourself","yourselves","z",
	"zero"}));


	private static HashSet<String> punctuationFilterWords = new HashSet<String>(Arrays.asList(new String[]{
			"!","'","[","]","[]","{","}","(",")","[(",")]","<",">",">>","<<",":",
			"-","...","?","\"","\"\"","'","''",";","/","&",".","@","#",
			"$","%","^","*","+","`","~",","}));
}
